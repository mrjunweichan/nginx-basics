...
```sh
curl -X POST -d '{"10.1.1.5":"1"}' -s 'http://localhost:8080/api/7/http/keyvals/allowlist_zone'
```
  
**Update Deny List Key Value Store**  
```sh
curl -X PATCH -d '{"10.1.1.5":"0"}' -s 'http://localhost:8080/api/7/http/keyvals/allowlist_zone'
```
